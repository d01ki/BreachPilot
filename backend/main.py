...
from backend.report.enhanced_report_generator import EnhancedProfessionalReportGenerator
import io
...
enhanced_report_generator = EnhancedProfessionalReportGenerator()
...
@app.post("/api/scan/{session_id}/enterprise-report")
async def generate_enterprise_report(session_id: str):
    """Generate comprehensive enterprise-grade security assessment report"""
    try:
        logger.info(f"Generating enterprise report for session: {session_id}")
        
        # Get session and extract target IP
        session = orchestrator._get_session(session_id)
        target_ip = session.target_ip
        
        # Generate enterprise report using enhanced generator
        enterprise_report_data = enhanced_report_generator.generate_enterprise_report(target_ip, session)
        
        return {
            "session_id": session_id,
            "target_ip": target_ip,
            "report_type": "enterprise",
            "report_data": enterprise_report_data,
            "download_urls": {
                "html": enterprise_report_data.get("report_files", {}).get("html_path", ""),
                "pdf": enterprise_report_data.get("report_files", {}).get("pdf_path", ""),
                "word": enterprise_report_data.get("report_files", {}).get("word_path", ""),
                "json": enterprise_report_data.get("report_files", {}).get("json_path", "")
            },
            "report_metadata": enterprise_report_data.get("report_metadata", {}),
            "executive_summary": enterprise_report_data.get("executive_summary", {}),
            "risk_assessment": enterprise_report_data.get("risk_assessment", {}),
            "business_case": enterprise_report_data.get("business_case", {}),
            "report_generated": True,
            "timestamp": datetime.now().isoformat()
        }
        
    except Exception as e:
        logger.error(f"Enterprise report generation failed for session {session_id}: {e}")
        raise HTTPException(status_code=500, detail=f"Enterprise report generation failed: {str(e)}")

@app.get("/api/reports/enterprise/{target_ip}")
async def get_enterprise_reports(target_ip: str):
    """Get all available enterprise reports for a target IP"""
    try:
        logger.info(f"Listing enterprise reports for target: {target_ip}")
        
        # Find all enterprise report files
        enterprise_reports = []
        report_patterns = [
            f"enterprise_security_report_{target_ip}_*",
            f"enterprise_assessment_{target_ip}_*"
        ]
            ...
            for file_path in files:
                if file_path.exists():
                    ...
                    enterprise_reports.append({
                        ...
                        "download_url": f"/api/reports/download/{file_type}/{target_ip}",
                        "path": str(file_path)
                    })
        ...
        return {
            ...
            "count": len(enterprise_reports),
            "report_types": list(set(r["type"] for r in enterprise_reports))
        }
        
    except Exception as e:
        logger.error(f"Failed to list enterprise reports for {target_ip}: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def download_report_api(report_type: str, target_ip: str):
    ...
    try:
        ...
        if report_type not in ['html', 'pdf', 'json', 'md', 'docx']:
            raise HTTPException(status_code=400, detail="File type must be html, pdf, json, md, or docx")
